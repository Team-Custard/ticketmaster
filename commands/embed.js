...
const { PermissionFlagsBits, EmbedBuilder, Colors, ActionRowBuilder, ButtonBuilder, ButtonStyle, ChatInputCommandInteraction, ComponentType, ModalBuilder, TextInputBuilder, TextInputStyle, Message, InteractionContextType } = require("discord.js");
...
class PingCommand extends Command {
  ...
  async runEmbedBuilder(interaction) {
    ...
    .setDescription("If you need to open a ticket, press the button below to get started.")
    .setFooter({ text: 'Powered by ticketmaster', iconURL: 'https://cdn.discordapp.com/avatars/1341574214821609655/e934260af53e33cc80a3194a17aef3a7.webp?size=4096' })
    ...
    collector.on("collect", async (collected) => {
      ...
      if (collected.customId == "EmbedSave") {
        ...
        try {
          ...
          if (!embeddb) {
            if (db.embeds.length == 25) return collected.reply({content:'Too many embeds. You can only have up to 25 embeds.', ephemeral: true});
            ...
            await db.save();
            modalResponse.followUp({ content: `Saved! There's now ${db.embeds.length} / 25 embeds.`, ephemeral: true });
          } else {
            ...
            await db.save();
            modalResponse.followUp({ content: `Updated! There's ${db.embeds.length} / 25 embeds.`, ephemeral: true });
          }
        }
        catch (err) {
          console.error('Error executing delete field embed. ',err);
      ...
      if (collected.customId == "EmbedDelete") {
        ...
        try {
          ...
          if (embeddb) {
            ...
            modalResponse.followUp({ content: `Deleted. There's now ${db.embeds.length} / 25 embeds.`, ephemeral: true });
        } else modalResponse.followUp({ content: `That embed doesn't exist.`, ephemeral: true });
        }
        catch (err) {
          console.error('Error executing delete field embed. ',err);
      ...
      if (collected.customId == "EmbedList") {
        try {
          const db = await serverSettings.findById(interaction.guild.id).cacheQuery();
          await collected.reply({ content: `There are ${db.embeds.length} / 25 embeds registered in this server.\n\n### Embeds:\n${db.embeds.map(e => `\`${e.name}\``)}`, ephemeral: true });
        }
        catch (err) {
          console.error('Error executing delete field embed. ',err);
          interaction.followUp({ content: `${this.container.emojis.error} ${err}`, ephemeral: true });
