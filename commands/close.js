...
class PingCommand extends Command {
  ...
  registerApplicationCommands(registry) {
    registry.registerChatInputCommand((builder) =>
      builder.setName("close").setDescription("Closes the ticket")
  ...
  async chatInputRun(interaction) {
    ...
    const ticket = db.tickets.find(t => t.channel == interaction.channel.id && t.closed == false)
    if (!ticket) return await interaction.reply({
      embeds: [new EmbedBuilder()
      .setTitle(`:x: Error`)
      .setDescription(`Not a valid ticket or the ticket is already closed.`)
      .setColor(Colors.Red)],
      flags: ["Ephemeral"]
    });

    ticket.closed = true;
    await interaction.channel.permissionOverwrites.edit(ticket.creator, { SendMessages: false });
    await db.save();
    ...
    if (db.logs.audit) {
      ...
      if (logchannel) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `${interaction.member} has closed the ticket ${interaction.channel}`,
          )
    ...
    await interaction.reply({
      content: `${interaction.member} has closed the ticket.`,
